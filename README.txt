...
Директория docroot -- рудимент. Для back-приложения docroot не имеет смысла, поскольку оно в принципе не обстлуживает GET-запросы, привязанные к файловой системе. Пока, тем не менее, docroot используется как техническая директория для некоторых механизмов ядра.
...
Как вы хорошо знаете, у нас традиционно не использовались UNIQUE-индексы. Не то, чтобы от тотальной некомпетентности. Это было связано с организацией хранения данных, при которой актуальные (fake=0), удалённые (fake=-1) и недовведённые  (fake > 0) записи соседствуют в одной таблице. Обычно условие UNIQUE имеет смысл лишь для актуальных данных, а среди удалённых — как раз нельзя запрещать держать дубликаты, иначе системой будет невозможно подьзоваться. PostgreSQL уже очень давно имел как раз необходимую функциональность — CREATE INDEX ... WHERE; аналогичного эффекта можно добиться в Oracle через индексы по функциям, но мы изначально орментировались на MySQL, где ничего похожего не было. Сейчас приоритеты меняются, а MySQL постепенно доделывается в нужную сторону. В общем, в ядро ветви cut добавлена поддержка частичных UNIQUE-индексов, а в b8 (do_update_users / do_undelete_users) показано, как этим пользоваться — вместо vld_unique, недопустимого в многозадачном режиме.
...
* front/root/_/libs -- общий корень всех внешних библиотек, к которым, наряду со сторонними requirejs, jquery и less относятся и заменители наших Presentation.pm и TurboMilk.pm: core и tmilk;
...
Ещё в HTML-тексте можно заметить нестандартные (но допустимые) атрибуты с префиксом data-. Они используются при интерполяции данных с HTML-шаблон. Например, data-off убивает содержащий его элемент при истинном значении соответствующего поля, data-on -- убивает при ложном значении data-text и data-html вызывают соответствующие методы jQuery, а data-list тиражирует элемент по значению списку.
...
4) развесить обработчики событий.
...
Заметим также, что id сессии js-коду не виден: он передаётся между сервером ибраузером в виде httpOnly cookie. Клиентское приложение узнаёт об успешной аутентификации в ответе на запрос {type: 'sessions', action: 'create'} и записывает этот факт в sessionStorage. Далее для js-кода именно состояние sessionStorage является показателем того, представился ли пользователь.
